...
def perform_occlusion_analysis(model, image):
    ...
    minimal_patch = 56 # Can be changed
    ...
    def patch_partition(starting_point, current_patch_size, heatmap):
        ...
        better_patch_size_y = min(current_patch_size, image.shape[2] - y)
        better_patch_size_x = min(current_patch_size, image.shape[3] - x)
        ...
        if differentiate:
            ...
            for poss_y in [0, half_patch]:
                for poss_x in [0, half_patch]:
                    ...
                    if (new_y < image.shape[2] and new_x < image.shape[3]):
    ...
    for y in range(0, image.shape[2], initial_patch):
        for x in range(0, image.shape[3], initial_patch):
